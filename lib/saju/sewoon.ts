...
export function calculateSeWoon(saju?: SajuPillars): string {
  ...
  return `${detail.year}년 ${detail.pillar}년. 연간 ${detail.stem}(${detail.stemElement}), 연지 ${detail.branch}(${detail.branchElement})가 들어오며 ${detail.relation}으로 해석합니다. ${detail.note}`;
}
